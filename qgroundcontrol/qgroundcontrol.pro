...
# DEFINES：编译器C预处理器宏(-D选项)：定义一个 宏 放入生成的Makefile文件中
#例如：
...
#PWD 本变量指定指向当前文件被解析的目录的全路径。为了支持影子构建，编写工程文件时在源码树引用文件时会有用。
#OUT_PWD 本变量包含指向生成MakeFile文件的目录的全路径
#QMAKE 本变量包含qmake程序自己的名字，会放在生成的MakeFile文件中。
#QMAKESPEC 当生成MakeFile时，本变量包含qmake配置要使用的名称。
#使用QMAKESPEC环境变量会覆盖qmake配置。
#注意，由于qmake读取工程文件的方式，在工程文件内设置QMAKESPEC变量会没有效果。
message(PWD=$${PWD})
#Project MESSAGE: PWD=/media/zcq/office/QT5_Code/zcq-qgroundcontrol
message(OUT_PWD=$${OUT_PWD})
#Project MESSAGE: OUT_PWD=/media/zcq/office/QT5_Code/qt_code_test/build-qgroundcontrol-Desktop_Qt_5_12_9_GCC_64bit-Release
message(QMAKE=$${QMAKE})
#Project MESSAGE: QMAKE=
message(QMAKESPEC=$${QMAKESPEC})
#Project MESSAGE: QMAKESPEC=/opt/Qt5.12.9/5.12.9/gcc_64/mkspecs/linux-g++

# 测试给定文件名的文件是否存在。如果文件存在，函数成功；否则，失败。
# 如果文件名是一个正则表达式，如果有任何文件匹配成功，则函数执行成功。
exists($${OUT_PWD}/qgroundcontrol.pro) {
    error("You must use shadow build (e.g. mkdir build; cd build; qmake ../qgroundcontrol.pro).")
}

message(Qt version $$[QT_VERSION])
#Project MESSAGE: Qt version 5.12.9
...
TARGET   = QGroundControl  # 生成 app 的名字
TEMPLATE = app # 编译产物类型
...
QGC_APP_NAME        = "zcq_QGroundControl"
QGC_ORG_NAME        = "zcq_QGroundControl.org"
QGC_ORG_DOMAIN      = "zcq_org.qgroundcontrol"
QGC_APP_DESCRIPTION = "(zcq) Open source ground control app provided by QGroundControl dev team"
QGC_APP_COPYRIGHT   = "(zcq) Copyright (C) 2019 QGroundControl Development Team. All rights reserved."

equals(0, 1) {
	message(QGC_APP_NAME=$${QGC_APP_NAME})
	message(QGC_ORG_NAME=$${QGC_ORG_NAME})
	message(QGC_ORG_DOMAIN=$${QGC_ORG_DOMAIN})
	message(QGC_APP_DESCRIPTION=$${QGC_APP_DESCRIPTION})
	message(QGC_APP_COPYRIGHT=$${QGC_APP_COPYRIGHT})
}
...
LinuxBuild {
	...
	message("LinuxBuild ---- CONFIG=$${CONFIG}")
}
...
DebugBuild {
	...
	message("DebugBuild : CONFIG -= qtquickcompiler")
} else {
    CONFIG += qtquickcompiler
	message("DebugBuild : CONFIG += qtquickcompiler")
}
...
#
message(---- External library configuration)
...
CustomBuild {
	...
	DEFINES += QGC_APPLICATION_NAME=\"\\\"zcq_QGroundControl\\\"\"
	DEFINES += QGC_ORG_NAME=\"\\\"zcq_QGroundControl.org\\\"\"
	DEFINES += QGC_ORG_DOMAIN=\"\\\"zcq_org.qgroundcontrol\\\"\"
    RESOURCES += \
