...
Item {
	...
	Item {
		...
		ToolStrip {
			...
			anchors.leftMargin: 0             //isInstrumentRight() ? _toolsMargin : undefined
			...
			anchors.topMargin:  ScreenTools.hufei_y + 50    //_toolsMargin
			anchors.top:        parent.top
			z:                  _mapAndVideo.z + 4
	...
	/**************************************************************/
	Image {
		id: zcq_Mavlink_recv_Image
		visible: activeVehicle ? true : false
		source: visible ? "qrc:/MyImages/MyImages/Pikachu.ico" :
						  "qrc:/MyImages/MyImages/PokeBall.ico"
		sourceSize.width: 30
		sourceSize.height: 30
		anchors.right: parent.right
		anchors.rightMargin: 0
		anchors.bottom: parent.bottom
		anchors.bottomMargin: zcq_Mavlink_recv_test.height + 50
		MouseArea {
			anchors.margins: 0
			anchors.fill: parent
			drag.target: zcq_Mavlink_recv_test
			drag.axis: Drag.XAndYAxis
			onClicked: {
				zcq_Mavlink_recv_test.zcq_recv_visible=!zcq_Mavlink_recv_test.zcq_recv_visible
			}
		}
	}
	/* 接收显示 */
	Rectangle {
		id: zcq_Mavlink_recv_test
		property bool zcq_recv_visible: false
		visible: zcq_recv_visible
		//visible: activeVehicle ? true : false
		anchors.right: zcq_Mavlink_recv_Image.right
		anchors.rightMargin: 0
		anchors.top: zcq_Mavlink_recv_Image.bottom
		anchors.bottomMargin: 0
		// anchors.verticalCenter: parent.verticalCenter
		// anchors.horizontalCenter: parent.horizontalCenter
		height:                     colSendRect.height
		width:                      colSendRect.width + 4 * 2
		radius:                     2
		color:                      "transparent"
		opacity: 1
		border.color:				"white"
		...
		Column {
			...
			Row {
				...
				QGCLabel {
					anchors.verticalCenter: parent.verticalCenter
					text:                   activeVehicle.battery_voltage
					color:                  "pink"
					//horizontalAlignment: Text.AlignHCenter
					verticalAlignment: Text.AlignVCenter
				}
			...
			Row {
				...
				GridLayout {
					...
					Repeater {
						model: activeVehicle.motor_current
						QGCLabel {
				...
				GridLayout {
					...
					Repeater {
						model: activeVehicle.motor_temperature
						QGCLabel {
							Layout.alignment: Qt.AlignRight
							text: modelData
							color:                  "pink"
				...
				GridLayout {
					...
					Repeater {
						model: activeVehicle.motor_speed
						QGCLabel {
							Layout.alignment: Qt.AlignRight
							text: modelData
							color:                  "pink"
