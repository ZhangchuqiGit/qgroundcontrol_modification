...
Item {
	...
	property bool   _disableDueToArmed:     vehicleComponent ?
												(!vehicleComponent.allowSetupWhileArmed && _vehicleArmed) : false
	// FIXME: The _vehicleIsRover checkl is a hack to work around https://github.com/PX4/Firmware/issues/10969
	property bool   _disableDueToFlying:    vehicleComponent ?
												(!_vehicleIsRover && !vehicleComponent.allowSetupWhileFlying
		...
		RowLayout {
			...
			ColumnLayout {
				...
				QGCLabel {
					...
					text:               !setupView.enabled ?
											_pageTitle + "<font color=\"red\">"
											+ qsTr(" (Disabled while the vehicle is %1)").arg(_disableReason)
											+ "</font>" : _pageTitle
