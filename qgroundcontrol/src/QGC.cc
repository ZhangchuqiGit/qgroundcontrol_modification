...
void initTimer() // 记录启动时间
{
	gBootTime = groundTimeMilliseconds();
	zcq_debug_echo(__FILE__, __func__, __LINE__,
				   "记录启动时间(自1970-01-01T00:00:00(UTC))毫秒 " << gBootTime)
}

quint64 bootTimeMilliseconds() // 获取自启动以来的时间（毫秒）
{
...
{
	// 返回自 1970-01-01T00:00:00(UTC)以来，所经过的毫秒
...
qreal groundTimeSeconds()
{
	...
	//return static_cast<quint64>(QDateTime::currentSecsSinceEpoch())*1.0;
}
