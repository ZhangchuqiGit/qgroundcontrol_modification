...
{
	this->_app = this;
	this->_msecsElapsedTime.start();// 一种快捷的计算流逝时间的方法。开始计时

#ifdef Q_OS_LINUX
#ifndef __mobile__
	if (!_runningUnitTests) {
		if (getuid() == 0) { /* zcq($USER) UID=1000; root UID=0; other UID */
			/* 退出并输出错误信息 */
			_exitWithError(QString(
							   tr("You are running %1 as root.(see $cat /etc/group) "
		...
		zcq_debug_echo(__FILE__, __FUNCTION__, __LINE__,
					   "qgcApp()->applicationName(): " << qgcApp()->applicationName());
	...
	QNetworkProxyFactory::setUseSystemConfiguration(true);//使用IE系统代理设定
	...
	_missingParamsDelayedDisplayTimer.setSingleShot(true/*非单次计时器*/);
	/* 设置计时器的时间间隔 */
	_missingParamsDelayedDisplayTimer.setInterval(_missingParamsDelayedDisplayTimerTimeout);

	connect(&_missingParamsDelayedDisplayTimer, &QTimer::timeout, this, &QGCApplication::_missingParamsDisplay);//超时

	/** ***************************************
	 创建一个QSettings对象，必须传入应用程序名称，及公司名称 **/
	...
	if (_runningUnitTests) {
		...
		this->setApplicationName(QGC_APPLICATION_NAME/*zcq_QGroundControl*/);
	}
	this->setOrganizationName(QGC_ORG_NAME/*zcq_QGroundControl.org*/);///公司名称
	this->setOrganizationDomain(QGC_ORG_DOMAIN/*zcq_org.qgroundcontrol*/);
	this->setApplicationVersion(QString(GIT_VERSION));
	zcq_debug_echo(__FILE__, __FUNCTION__, __LINE__,
				   "GIT_VERSION=" << GIT_VERSION );// GIT_VERSION=None
	// Set settings format
	...
	zcq_debug_echo(__FILE__, __FUNCTION__, __LINE__,
				   "fClearSettingsOptions=" << fClearSettingsOptions /*false*/ );
	if (fClearSettingsOptions) {
		...
		if (settings.contains(_settingsVersionKey)) {
			zcq_debug_echo(__FILE__, __FUNCTION__, __LINE__,
						   "settings.value(_settingsVersionKey).toInt()="
						   << settings.value(_settingsVersionKey).toInt() /*9*/ );
			...
			zcq_debug_echo(__FILE__, __FUNCTION__, __LINE__,
						   "settings.allKeys().count()=" << settings.allKeys().count() );
			// Settings version key is missing and there are settings.
	...
	if (settings.contains(AppSettings::gstDebugLevelName)) {
		...
		zcq_debug_echo(__FILE__, __FUNCTION__, __LINE__, "gstDebugLevel=" << gstDebugLevel);
	}
	// Initialize Video Streaming
	initializeVideoStreaming(argc, argv, gstDebugLevel);

	_toolbox = new QGCToolbox(this);//加载各个工具组件
...
void QGCApplication::setLanguage()
{
	...
	qDebug() << "Loading localization for " << _locale.name();//zh_CN
	...
	if(_qmlAppEngine) _qmlAppEngine->retranslate();
...
void QGCApplication::_initCommon()//GC app 注册C++类到QML元对象中给QML使用
{
...
/* initForNormalAppBoot 初始化 MainWindow 主界面以及相关硬件设备 遥感之类的
最后同步 Settings 配置文件 */
bool QGCApplication::_initForNormalAppBoot()
{
	...
	_qmlAppEngine = toolbox()->corePlugin()->createRootWindow(this);//MainRootWindow.qml
...
{
	return QGCApplication::_app;/// *this
}
...
void QGCApplication::_missingParamsDisplay(void)//超时
{
