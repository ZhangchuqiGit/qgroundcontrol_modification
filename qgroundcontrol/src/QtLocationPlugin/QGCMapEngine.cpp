...
static const char* kDbFileName = "qgc_MapCache.db";
...
QGCMapEngine::_checkWipeDirectory(const QString& dirPath)
{
	...
	if (dir.exists(dirPath)) {
		_cacheWasReset = true;
		_wipeDirectory(dirPath);//QDir::rmdir(dirPath)
	}
}

//-----------------------------------------------------------------------------
void QGCMapEngine::_wipeOldCaches()//Delete old style caches (if present)
{
	QString oldCacheDir;
#ifdef __mobile__
	oldCacheDir =
			QStandardPaths::writableLocation(QStandardPaths::AppDataLocation)      +
			QLatin1String("/QGCMapCache55");
#else
	oldCacheDir =
			QStandardPaths::writableLocation(QStandardPaths::GenericCacheLocation) +
			QStringLiteral("/QGCMapCache55");
#endif
	_checkWipeDirectory(oldCacheDir);
#ifdef __mobile__
	oldCacheDir =
			QStandardPaths::writableLocation(QStandardPaths::AppDataLocation)      +
			QLatin1String("/QGCMapCache100");
#else
	oldCacheDir =
			QStandardPaths::writableLocation(QStandardPaths::GenericCacheLocation) +
			QStringLiteral("/QGCMapCache100");// home/zcq/.cache/QGCMapCache100
#endif
	_checkWipeDirectory(oldCacheDir);
}

//-----------------------------------------------------------------------------
void QGCMapEngine::init()//初始化QGC地图引擎
{
	...
	QString cacheDir =
			QStandardPaths::writableLocation(QStandardPaths::GenericCacheLocation) +
			QStringLiteral("/zcq_QGCMapCache" CACHE_PATH_VERSION);
	// home/zcq/.cache/QGCMapCache300
#endif
	if(!QDir::root().mkpath(cacheDir)) {
		qWarning() << "Could not create mapping disk cache directory: " << cacheDir;
		cacheDir = QDir::homePath() + QStringLiteral("/.zcq_qgc_mapscache/");
		if(!QDir::root().mkpath(cacheDir)) {
			qWarning() << "Could not create mapping disk cache directory: " << cacheDir;
			cacheDir.clear();
		}
	...
	if(!_cachePath.isEmpty()) {
		...
		zcq_debug("Map Cache in: " << _cachePath << "/" << _cacheFile);
	} else {
