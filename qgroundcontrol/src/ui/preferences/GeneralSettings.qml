...
Rectangle {
	...
	property real _margins:                     5 // ScreenTools.defaultFontPixelWidth
		...
		Item {
			...
			ColumnLayout {
				...
				Rectangle {
					...
					Item {
						...
						GridLayout {
							...
							QGCLabel {
								text:                   qsTr("Stream GCS Position")//流动地面站位置
					...
					Item {
						...
						ColumnLayout {
							...
							FactCheckBox {
								text:       qsTr("Mute all audio output")//静音所有音频输出
								fact:       _audioMuted
							...
							FactCheckBox {
								text:       qsTr("AutoLoad Missions")//自动加载任务
								fact:       _autoLoad
							...
							QGCCheckBox {
								id:         clearCheck
								text:       qsTr("Clear all settings on next start")//下次启动时清除所有设置
								checked:    false
								onClicked: {
								...
								MessageDialog {
									...
									title:              qsTr("Clear Settings")//清除设置
							...
							RowLayout {
								...
								QGCCheckBox {
									...
									text:       qsTr("Announce battery lower than")//电池低于该电量时提示
									checked:    _percentRemainingAnnounce.value !== 0
									onClicked: {
										if (checked) {
											_percentRemainingAnnounce.value =
													_percentRemainingAnnounce.defaultValueString
										} else {
					...
					RowLayout {
						...
						QGCLabel { text: qsTr("Application Load/Save Path") }//应用程序加载/保存路径
						QGCTextField {
				...
				QGCLabel {
					...
					text:       qsTr("Data Persistence")//数据持久化
					color: "deepskyblue"
				}
				Rectangle {
					...
					ColumnLayout {
						...
						FactCheckBox {
							...
							text:       qsTr("Disable all data persistence")//禁用所有数据持久化
				...
				QGCLabel {
					text:       qsTr("Telemetry Logs from Vehicle")//飞机中的数传日志
				...
				Rectangle {
					...
					ColumnLayout {
						...
						FactCheckBox {
							id:         promptSaveLog
							text:       qsTr("Save log after each flight")//每次飞行后保存日志
							fact:       _telemetrySave
						...
						FactCheckBox {
							id:         logIfNotArmed
							text:       qsTr("Save logs even if vehicle was not armed")//即使载具未解锁时也保存日志
				...
				Rectangle {
					...
					ColumnLayout {
						...
						FactCheckBox {
							id:             useCheckList
							text:           qsTr("Use Preflight Checklist")//使用起飞前检查清单
							fact:           _useChecklist
						...
						FactCheckBox {
							text:       qsTr("Virtual Joystick")//虚拟游戏手柄
						...
						GridLayout {
							...
							QGCLabel {
								text:                   qsTr("Guided Minimum Altitude")//引导模式下最小高度
				...
				Rectangle {
					...
					ColumnLayout {
						...
						RowLayout {
							...
							QGCLabel { text: qsTr("Default Mission Altitude") }//默认任务高度：
							FactTextField {
								Layout.preferredWidth:  _valueFieldWidth
