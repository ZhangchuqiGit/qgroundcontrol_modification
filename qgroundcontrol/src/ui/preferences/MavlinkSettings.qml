...
Rectangle {
	...
	property real _columnSpacing:       5 // ScreenTools.defaultFontPixelWidth
	...
	QGCFlickable {
		...
		Column {
			...
			Item {
				...
				QGCLabel {
					id:             gcsLabel
					text:           qsTr("Ground Station")//地面站
					font.family:    ScreenTools.demiboldFontFamily
					color: "deepskyblue"
				}
			}
			Rectangle {
				height:         gcsColumn.height + (ScreenTools.defaultFontPixelWidth * 2)
				width:          __mavlinkRoot.width * 0.8
				...
				Column {
					...
					Row {
						...
						QGCLabel {
							...
							text:               qsTr("MAVLink System ID:")//MAVLink 系统 ID：
						}
						QGCTextField {
							id:     sysidField
					...
					QGCCheckBox {
						text:       qsTr("Emit heartbeat")//发出心跳包
			...
			Item {
				...
				QGCLabel {
					id:             streamRatesLabel
					text:           qsTr("Telemetry Stream Rates (ArduPilot Only)")//遥测流速率(仅ArduPilot)
					font.family:    ScreenTools.demiboldFontFamily
					color: "deepskyblue"
				}
			}
...
//							anchors.baseline:   videoUrlField.baseline
			...
			Item {
				...
				QGCLabel {
					id:             logFilesLabel
					text:           qsTr("Saved Log Files")//保存的日志文件
					color: "deepskyblue"
					font.family:    ScreenTools.demiboldFontFamily
				}
			}
			Rectangle {
				height:         logFilesColumn.height + (ScreenTools.defaultFontPixelWidth * 2)
				width:          __mavlinkRoot.width * 0.8
				color:          qgcPal.windowShade
				anchors.margins: ScreenTools.defaultFontPixelWidth
				anchors.horizontalCenter: parent.horizontalCenter
