...
void Vehicle::_mavlinkMessageReceived(LinkInterface* link, mavlink_message_t message)
{
	...
	case MAVLINK_MSG_ID_VEHICLE_SAFETY_STATUS: /******** zcq ********/
		/******************* zcq QGC中接收函数 ************/
		_handle_ZcqMavlinkTestRct(message);
		break;
...
/******************* zcq QGC中接收函数 ************/
void Vehicle::_handle_ZcqMavlinkTestRct(mavlink_message_t& message)	/* zcq */
{
	mavlink_vehicle_safety_status_t vehicle_safety_status_t;
	mavlink_msg_vehicle_safety_status_decode(&message, &vehicle_safety_status_t);

	/******* *********/
	_time_usec=vehicle_safety_status_t.time_usec;

	_motor_current.clear();
	_motor_temperature.clear();
	_motor_speed.clear();
	for (int i=0; i < 16; i++) {
		_motor_current.append(QVariant::fromValue(vehicle_safety_status_t.motor_current[i]));
		_motor_temperature.append(QVariant::fromValue(vehicle_safety_status_t.motor_temperature[i]));
		_motor_speed.append(QVariant::fromValue(vehicle_safety_status_t.motor_speed[i]));
	}

	_battery_voltage=vehicle_safety_status_t.battery_voltage;

	//触发信号，QML前端中接收，可搜 activeVehicle.time_usec
	emit time_usecChanged(_time_usec);
	emit motor_currentChanged(_motor_current);
	emit motor_temperatureChanged(_motor_temperature);
	emit motor_speedChanged(_motor_speed);
	emit battery_voltageChanged(_battery_voltage);
}
/***************************************************/
