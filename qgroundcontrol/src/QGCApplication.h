 ...
 * dialogs, that are spawned in QML, when they are closed
 @简要介绍主要的应用和管理类。
	该类由main方法启动，并提供地面站应用程序的中央管理单元。
需要QApplication base来支持QtCharts模块。
这样我们就避免了在使用该模块时应用程序在5.12上崩溃。
注意：`lastWindowClosed`将由MessageBox弹出窗口和其他对话框发送，
这些对话框在关闭时在QML中生成 **/
/** QGC App 程序真正的入口地方(整个程序就运行) QGCApplication(argc, argv, false) */
...
private slots:
	void _missingParamsDisplay(void);//超时
...
private:
	...
	void _exitWithError (QString errorMessage);//退出并输出错误信息
	...
	QLocale _locale;
	bool _error = false;
	QElapsedTimer _msecsElapsedTime;//一种快捷的计算流逝时间的方法。
...
QGCApplication* qgcApp(void); ///return QGCApplication::_app(*this)
