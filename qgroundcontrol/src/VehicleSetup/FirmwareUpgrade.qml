	...
	Component {
		...
		ColumnLayout {
			...
			readonly property string plugInText:  "<big>"
												  + highlightPrefix + "Plug in your device" + highlightSuffix
												  + " via USB to " + highlightPrefix + "start" + highlightSuffix
												  + " firmware upgrade.</big>"
			readonly property string flashFailText: "If upgrade failed, make sure to connect "
													+ highlightPrefix + "directly" + highlightSuffix
													+" to a powered USB port on your computer, not through a USB hub. "
													+ "Also make sure you are only powered via USB "
													+ highlightPrefix + "not battery" + highlightSuffix + "."
			...
			readonly property string qgcUnplugText2: highlightPrefix
													 + "<big>Please unplug your Pixhawk and/or Radio from USB.</big>"
													 + highlightSuffix
			...
			///< true: running in special single firmware download mode
				...
				onBoardFound: {
					if (initialBoardSearch) {
						...
						if(availableDevices.length > 1) {
							...
							statusTextArea.append(qsTr("Detected [%1]: ").arg(availableDevices.length)
						...
						statusTextArea.append(highlightPrefix + qsTr("Found device")
											  + highlightSuffix + ": " + controller.boardType)
					...
					function accept() {
						if (_singleFirmwareMode) {
							...
							if (px4Flow) {
								...
								stack = apmFlightStack.checked ?
											FirmwareUpgradeController.AutoPilotStackAPM :
											FirmwareUpgradeController.AutoPilotStackPX4
								if (apmFlightStack.checked) {
									if (firmwareBuildType === FirmwareUpgradeController.CustomFirmware) {
										...
										if (controller.apmFirmwareNames.length === 0) {return}
						...
						Column {
							...
							QGCLabel {
								...
								text:       (_singleFirmwareMode
											 || !QGroundControl.apmFirmwareSupported) ?
												_singleFirmwareLabel : (px4Flow ? _px4FlowLabel : _pixhawkLabel)
							...
							QGCLabel {
								...
								visible:        !controller.downloadingFirmwareList
												&& (QGroundControl.apmFirmwareSupported
													&& controller.apmFirmwareNames.length === 0)
							}
							QGCComboBox {
							...
							QGCComboBox {
								...
								visible:        showFirmwareTypeSelection
								textRole:       "text"
								model:          _singleFirmwareMode ?
													singleFirmwareModeTypeList :
													(px4Flow ? px4FlowTypeList : firmwareBuildTypeList)
								...
								onActivated: {
									...
									if (model.get(index).firmwareType === FirmwareUpgradeController.BetaFirmware) {
										...
										firmwareVersionWarningLabel.text = qsTr("WARNING: BETA FIRMWARE. ")
												...
												+ qsTr("Although it has received FLIGHT TESTING, it represents actively changed code. ") + qsTr("Do NOT use for normal operation.")
