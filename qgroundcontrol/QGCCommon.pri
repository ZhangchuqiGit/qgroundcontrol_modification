...
#	...
#}
#通过代码块作用域可以实现类似编程语言中 IF 语句的行为，支持条件控制结构
#只有条件为true时，括号内的赋值才会有效。在这个例子中，特殊变量 linux 必须被设置。
#在 linux 平台上， linux 会自动设置
linux {
...
ReleaseBuild {
	...
	message( "QDebug 版本是可以输出日志信息的，但在 Release 中输入日志的文件信息和行数为空。" )
	message( "原因是，Qt 在 Release 版本默认舍弃了文件信息、行数等信息。" )
	message( "解决方案：DEFINES += QT_MESSAGELOGCONTEXT " )
	message( "Release 版本默认舍弃 DEFINES += QT_NO_DEBUG QT_NO_MESSAGELOGCONTEXT " )
